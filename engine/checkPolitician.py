...
modelfile = resroot+"/resources/NLP_polititian_model.pkl"
mapfile   = resroot+"/resources/NLP_polititian_wordmap.pkl"
...
def trainSimpleNLPModel(politicians,normals) :
    ...
    print "Threshold     = ", (np.mean(pol_scores) + np.mean(norm_scores))/2.
    return (np.mean(pol_scores) + np.mean(norm_scores))/2.
...
if __name__ == "__main__" :
    ...
    for row in data.iterrows() :
        ...
        pickle.dump(backup,open("backup.pkl","w"))
